...
#
# Added that two players can play against each other, or, as before,
# one player can play against the cumputer.
#
...
class c:  # Gather the constants here (in C they were #define s)
    X = 60  # The board width
    Y = 15  # its high
    D =  9  # Max sensibility of the sonar
    SONAR_DEVICES = 20
    MAX_PLAYERS = 2  # <<<<<< Change here to allow multiple players
...
up to a distance of {c.D} spaces. Try to collect all 3 chests before running out of
sonar devices. Good luck!

One player can play against the computer, or two players against each other.
(Hint: If you allow to enter more than 2 players, it should work, too,)
...
while True:
    ...
    # Get number of players, one or two
    while True:  # Break out when got the number
        _n = input(f'Enter the number of players (max {c.MAX_PLAYERS}): ')
        if _n.isdecimal():
            np = int(_n)
            if 1 <= np <= c.MAX_PLAYERS:
                break

    sonarDevices = c.SONAR_DEVICES
    ...
    while sonarDevices > 0:
        ...
        if len(theChests) == 0:
            ...
            for x, y in chestsBackup:
                theBoard[x][y] = 'C'

            drawBoard(theBoard)
            print('You have found all the sunken treasure chests! They were at: ', end='')
